...
async def async_setup_entry(
    ...
    fan_lights: list[Entity] = [
        BondLight(hub, device)  # , "fan_light")
    ...
    fp_lights: list[Entity] = [
        BondLight(hub, device, "light")  # bpup_subs,
        for device in hub.devices
    ...
    lights: list[Entity] = [
        ...
        for device in hub.devices
        if DeviceType.is_light(device.type) and not DeviceType.is_fan(device.type)
    ]
...
class BondLight(BondBaseLight, BondEntity, LightEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if brightness := kwargs.get(ATTR_BRIGHTNESS):
            ...
            await self._hub.bond.action(self._device.device_id, Action.light_on())
