...
async def async_setup_entry(
    ...
    """Set up Hub cover (Shades, Drapes, Blinds) devices."""
    ...
    async_add_entities(
        ...
        if DeviceType.is_shades(device.type)
        if DeviceType.is_drapes(device.type)
        if DeviceType.is_blinds(device.type)
        # if device.type == DeviceType.SHADES
    )


class BondCover(BondEntity, CoverEntity):
    """Representation of a Hub cover (blinds)."""

    _attr_device_class = CoverDeviceClass.SHADE

    def __init__(self, hub: BondHub, device: BondDevice) -> None:
        """Create HA entity representing Hub blinds."""
        super().__init__(hub, device)
        supported_features = 0
        if self._device.supports_set_position():
            supported_features |= CoverEntityFeature.SET_POSITION
        if self._device.supports_up():
            supported_features |= CoverEntityFeature.OPEN
        if self._device.supports_down():
        ...
        if self._device.supports_tilt_up():
            supported_features |= CoverEntityFeature.OPEN_TILT
        if self._device.supports_tilt_down():
            supported_features |= CoverEntityFeature.CLOSE_TILT
