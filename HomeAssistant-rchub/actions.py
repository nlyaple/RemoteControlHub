...
# Tuple for fan speeds from stopped to max
# API_FAN_SPEEDS = ("Stop", "Speed 1", "Speed 2", "Speed 3", "Speed 4", "Speed 5", "Speed 6")
# API_OPEN_PERCENT = ("Qtr", "Half", "Third")


class DeviceActions:
    """Nelsony defined device types."""

    devTypes = [
        ("Not Used"),
        ("Up", "Stop", "Down"),
        ("Open", "Stop", "Close"),
        ("Open", "Stop", "Close", "Qtr", "Half", "Third"),
        (
            "Speed 1",
            "Speed 2",
            "Speed 3",
        ...
        ("Up", "Stop", "Down", "Tilt Up", "Stop Tilt", "Tilt Down"),
    ]
...
class Action:
    ...
    def __init__(self, name: str, argument: Any = None) -> None:
        self._name = name
        if self._name == Action.SET_STATE_BELIEF:
            self._argument = {} if not argument else argument
        else:
            if self._name == Action.SET_SPEED:
                self._name = name + str(argument)
    ...
    @staticmethod
    def light_on() -> "Action":
        """Turn light on (usually power)."""
        return Action(Action.LIGHT_ON)
    ...
    def light_off() -> "Action":
        """Turn light off (usually power)."""
        return Action(Action.LIGHT_OFF)
    ...
    @staticmethod
    def tilt_down() -> "Action":
        """Tilt close cover."""
        return Action(Action.TILT_DOWN)
