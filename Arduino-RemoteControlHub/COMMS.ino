...
#ifndef FILESYS_ERROR
bool    FILESYS_ERROR = false;
#endif //  FILESYS_ERROR

#ifdef INTERVAL_SENSORS
// Firmware update variables.
int             totalLength;       //total size of firmware
int             currentLength = 0; //current size of written firmware
#endif // INTERVAL_SENSORS
...
//#endif // LITTLE_FS vs. SPIFFS
    
#ifndef INTERVAL_SENSORS    
    checkForFileUpdates();
#endif // INTERVAL_SENSORS    
...
readEEPROMStore()
{
       ...
       (EEPROM.read(VALID_OFFSET + 1) == VALID_SIG1))
    {
        ...
        userConfig = EEPROM.read(UFLAGS_OFFSET + 1);
        ...
        {
            email_address[i] = EEPROM.read(EMAIL_OFFSET + i);
        }
#endif // Im OK Occupancy Sensor plaform
    ...
    else
    {
        ...
        userConfig = UC_BAT_VOLTAGE | UC_BME_TEMP | UC_ONEWIRE_TEMP;
        EEPROM.write(UFLAGS_OFFSET + 1, userConfig);
        for (int i= 0; i < SSID_LENGTH; i++)
        {
...
connectWifi()
{
    ...
    if (userFlags & UF_WIFI_ON)
    {
        ...
        {
            Serial.print("connectWifi, trying AP: ");
            Serial.println(ssid);
            WiFi.mode(WIFI_STA);  // Old WiFi library
            delay(500);
            WiFi.disconnect(true);
            delay(500);
            adHocMode = false;
...
#ifdef SOIL_SENSOR
            sprintf((char *)&localSSID[0], "NelsonySoilSense%d", devNum);
#endif // Weather Sensor
#ifdef MULTI_SENSOR
            sprintf((char *)&localSSID[0], "MultiSense%d", devNum);
#endif // Multi Sensor plaform
#ifdef IM_OK
            sprintf((char *)&localSSID[0], "Im_OK-0%d", devNum);
#endif // Im OK Occupancy Sensor plaform

            sprintf((char *)&password[0], "ConnectMe");  // devId);
            Serial.println((char *)&localSSID[0]);
            WiFi.softAP((char *)&localSSID[0], &localSSID[0]);  //devId);
            WiFi.softAP((char *)&localSSID[0], &password[0]); 
            //if (WiFi.beginAP((char *)&localSSID[0]) == WL_AP_LISTENING) 
            //{
                //sIPAddr = WiFi.softAPIP().toString();
                IPAddress sIPAddr = WiFi.localIP();
                Serial.print("IP address: ");
                Serial.println(sIPAddr);
                //sprintf((char *)&password[0], "%s", &localSSID[0]);  // devId);
...
#endif // Remote Control Hub        
#ifdef SOIL_SENSOR
        if (MDNS.begin("Nelsony-SoilSensor")) 
#endif // Soil Sensor        
#ifdef MULTI_SENSOR
        if (MDNS.begin("Nelsony-MultiSensor")) 
#endif // Soil Sensor        
#ifdef IM_OK
        if (MDNS.begin("ImOK"))
#endif // Occupancy Sensor plaform
#ifdef CO_LPG
        if (MDNS.begin("CO_LPG"))
#endif // CO LPG Sensor platform
        {
            gMDNSInit = true;
...
findService()
{
    ...
    if (!gMDNSInit)
    {
        ...
        {
            gMDNSInit = true;
            MDNS.addService("MultiSense", "tcp", 80);
        }
        else
        {
            Serial.println("mDNS failed to start");
    ...
    if (!gServiceFound)
    {
        while(retries--)
        {
            // Reset the Watchdog timer
            esp_task_wdt_reset();
            nbrServices = MDNS.queryService("hub", "tcp");
            if (nbrServices == 0) 
            {
...
/*!
    ...
    \note  Assumes that the gServerIP and gServerPort are populated
*/
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
int
checkFOTA(
    )
{
    ...
    if ((WiFi.status() == WL_CONNECTED)) 
    {
        ...
        sprintf(pURL, "http://%s:%i/api/update?devID=%s&version=%s", 
            gServerIP, gServerPort, deviceID, VERSION_NUM);
        Serial.print("Connecting to update service at: ");
        Serial.println(pURL);
        
#if 0 //def REFER_CONTROLLER        
        else
        {
